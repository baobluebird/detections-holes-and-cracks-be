...
const createCodeVerifyEmail = async (email) => {
    try {
        // Kiểm tra định dạng email
        if (!isValidEmail(email)) {
            return {
        ...
        // Gửi email với mã
        await EmailService.sendEmailVerify(email, code);
...
const verifyCodeEmail = async (codeId, code) => {
    try {
        // Kiểm tra codeId
        if (!isValidObjectId(codeId)) {
            return {
...
const getEmail = async (codeId) => {
    try {
        ...
        console.error(`Error in getEmail: ${error.message || error}`);
        return {
            status: 'ERR',
            message: error.message || 'Get email failed'
        };
    }
}
