...
mongoose.connect(process.env.MONGODB_URI)
  ...
  app.post('/api/detection/create', upload.single('image'), async (req, res) => {
    try {
      ...
      if (!typeDetection || !location || !image || !description || !userId || !address) {
        return res.status(200).json({
            status: 'ERR',
            message: 'The input is required'
        })
    }
    const response = await DetectionService.createDetection(typeDetection, location, image, description, userId, address)
